...
s = Settings(1001,500,1.0,"hyperbolic",SolverType); # Give the number of discretisation points for spatial domain and velocity domain as input i.e., Nx and Nv
# The input parameter for setting the matrices for angular discretisation are the same for both Pn and Sn solver
s.Tend = 1.0;
Solver = solver(s);
@time t, rhoDLR, gDLR = solveMMDLRA_SnIMEX(Solver);
...
# @time t, rho,g = solveFullProblem_Pn(Solver);
...
# ax1.plot(Solver.x, rho, label="PN for MM");
ax1.plot(Solver.x, rhoDLR, label="UI with PN for MM");
